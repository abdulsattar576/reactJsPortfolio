        ...
        >
          {services.map((service, index) => (
            <motion.div
              ...
              variants={item}
              whileHover={{ y: -10 }}
              className="bg-gray-800/50 backdrop-blur-md p-8 rounded-xl border border-gray-700 hover:border-purple-500/50 transition-all duration-300 group"
            >
              <div className={`${service.color} mb-6 text-4xl group-hover:scale-110 transition-transform`}>
                {service.icon}
              </div>
              <h3 className="text-xl font-bold mb-3">{service.title}</h3>
              <p className="text-gray-400">{service.description}</p>
              <div className="mt-6 pt-4 border-t border-gray-700 group-hover:border-purple-500/50 transition-all">
